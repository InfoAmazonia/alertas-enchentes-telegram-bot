...
public class Main {
	...
	@Bean
	public CommandLineRunner demo(AlertRepository alertRepo, ConversationRepository conversationRepo) {
		return (args) -> {
			TelegramBotsApi telegramBotsApi = new TelegramBotsApi();
			try {
				telegramBotsApi.registerBot(new AlertaEnchentesHandler(alertRepo, conversationRepo));
